...
public abstract class PieceDefinition : MonoBehaviour {
	...
	public virtual void checkPattern(int xPos, int yPos, int checkNum){
		...
		case BoardType.Hexagon :
			...
			foreach(Board _board in gm.board[xPos,yPos].getAllBoardInDirection(BoardDirection.TopLeft)){
				if ( _board.canBeMatched() && _board.piece.slotNum == slotNum) {
					linkedCubesDiagTLBR.Add(_board); // remember this board box
					matchingDiagTLBR++; // increase linked counter
				} else {
					break; // exit loop as no more match this side...
				}
			}
			foreach(Board _board in gm.board[xPos,yPos].getAllBoardInDirection(BoardDirection.BottomRight)){
				if ( _board.canBeMatched() && _board.piece.slotNum == slotNum) {
					linkedCubesDiagTLBR.Add(_board); // remember this board box
					matchingDiagTLBR++; // increase linked counter
				} else {
			...
			if ( matchingDiagTLBR < 2 ) { // means less than 2 similar cube in the row
				linkedCubesDiagTLBR.Clear(); // forget the marked boxes
			}

			// topRight <> bottomLeft check...
			foreach(Board _board in gm.board[xPos,yPos].getAllBoardInDirection(BoardDirection.TopRight)){
				if ( _board.canBeMatched() && _board.piece.slotNum == slotNum) {
					linkedCubesDiagTRBL.Add(_board); // remember this board box
					matchingDiagTRBL++; // increase linked counter
				} else {
					break; // exit loop as no more match this side...
				}
			}
			foreach(Board _board in gm.board[xPos,yPos].getAllBoardInDirection(BoardDirection.BottomLeft)){
				if ( _board.canBeMatched() && _board.piece.slotNum == slotNum) {
					linkedCubesDiagTRBL.Add(_board); // remember this board box
					matchingDiagTRBL++; // increase linked counter
				} else {
			...
			if ( matchingDiagTRBL < 2 ) { // means less than 2 similar cube in the row
				linkedCubesDiagTRBL.Clear(); // forget the marked boxes
			}
			break;
		}
