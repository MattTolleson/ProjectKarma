...
public class PieceTracker : MonoBehaviour {
	...
	void OnMouseEnter() {
		if(isBeingDragged && dragOrigin != arrayRef){
			if( gm.iBoard(dragOrigin).allNeighbourBoards.Contains( gm.iBoard(arrayRef) ) ){ // a neighbour board
	...
	// function to cancel the drag once user lets go...
	void OnMouseUp(){
		isBeingDragged = false; // cancels the drag
	}
	
	void OnMouseUpAsButton(){ // mouse click
