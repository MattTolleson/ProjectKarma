...
public class BombPiece : PieceDefinition {
	...
	public override bool matchConditions (int xPos, int yPos, List<Board> linkedCubesX, List<Board> linkedCubesY, List<Board> linkedCubesTRBL, List<Board> linkedCubesTLBR){
		...
		if ( chainMatch > 1 ) { // + or T or L-type match special pieces
	...
	IEnumerator doPowerMergeX(int[] posA, int[] posB,int radius){
		...
		case BoardType.Hexagon :
			gm.animScript.doAnim(animType.ARROWTLBR,posB); // perform anim
			gm.animScript.doAnim(animType.ARROWTRBL,posB); // perform anim
			delay = 0f; // reset the delay
			destroyThis = false; // help to skip the first board in the list...
			// the TopLeft of this board...
			foreach(Board _board in gm.iBoard(posB).getAllBoardInDirection(BoardDirection.TopLeft) ){
				if(destroyThis) gm.destroyInTime(_board.arrayRef,delay,scorePerCube);
				destroyThis = true;
				delay += delayIncreament;
			}
			...
			foreach(Board _board in gm.iBoard(posB).getAllBoardInDirection(BoardDirection.TopRight) ){
				if(destroyThis) gm.destroyInTime(_board.arrayRef,delay,scorePerCube);
				destroyThis = true;
				delay += delayIncreament;
			}
			...
			foreach(Board _board in gm.iBoard(posB).getAllBoardInDirection(BoardDirection.BottomLeft) ){
				if(destroyThis) gm.destroyInTime(_board.arrayRef,delay,scorePerCube);
				destroyThis = true;
				delay += delayIncreament;
			}
			...
			foreach(Board _board in gm.iBoard(posB).getAllBoardInDirection(BoardDirection.BottomRight) ){
				if(destroyThis) gm.destroyInTime(_board.arrayRef,delay,scorePerCube);
				destroyThis = true;
				delay += delayIncreament;
			}
		...
		case BoardType.Square :
			gm.animScript.doAnim(animType.ARROWVH,posB); // perform anim
			delay = 0f; // reset the delay
			destroyThis = false; // help to skip the first board in the list...
