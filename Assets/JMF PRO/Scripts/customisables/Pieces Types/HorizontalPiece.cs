...
public class HorizontalPiece : PieceDefinition {
	...
	void doHorizontalPower(int[] arrayRef){
		...
		case BoardType.Hexagon:// hex type horizontal power
			gm.animScript.doAnim(animType.ARROWTLBR,arrayRef[0],arrayRef[1]); // perform anim
			gm.animScript.doAnim(animType.ARROWTRBL,arrayRef[0],arrayRef[1]); // perform anim
			gm.audioScript.arrowSoundFx.play(); // play arrow sound fx
			
			// the TopLeft of this board...
			foreach(Board _board in gm.iBoard(arrayRef).getAllBoardInDirection(BoardDirection.TopLeft) ){
				gm.destroyInTime(_board.arrayRef,delay,scorePerCube);
				delay += delayIncreament;
			}
			delay = 0f; // reset the delay
			// the TopRight of this board...
			foreach(Board _board in gm.iBoard(arrayRef).getAllBoardInDirection(BoardDirection.TopRight) ){
				gm.destroyInTime(_board.arrayRef,delay,scorePerCube);
				delay += delayIncreament;
			}
			delay = 0f; // reset the delay
			// the BottomLeft of this board...
			foreach(Board _board in gm.iBoard(arrayRef).getAllBoardInDirection(BoardDirection.BottomLeft) ){
				gm.destroyInTime(_board.arrayRef,delay,scorePerCube);
				delay += delayIncreament;
			}
			delay = 0f; // reset the delay
			// the BottomRight of this board...
			foreach(Board _board in gm.iBoard(arrayRef).getAllBoardInDirection(BoardDirection.BottomRight) ){
				gm.destroyInTime(_board.arrayRef,delay,scorePerCube);
				delay += delayIncreament;
			}
			break;
